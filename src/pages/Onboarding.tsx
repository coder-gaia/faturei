  ...
  async function finish() {
    ...
    try {
      ...
      if (sessionError || !session?.user) {
        console.error("Erro ou ausência de sessão ativa:", sessionError)
        setError('Sessão expirada ou não encontrada. Faça login novamente.')
        navigate('/login')
        return
      }
      ...
      console.log("Iniciando salvamento no banco para o ID:", userId)
      ...
      if (profileError) {
        console.error("Erro específico na tabela profiles:", profileError.message)
        setError(`Erro no perfil: ${profileError.message}`)
        return
      }

      const dasRecords = generateYearlyDAS(userId, data.activity_type)
      const { error: dasError } = await supabase
        .from('das_payments')
        .upsert(dasRecords, { onConflict: 'user_id,year,month', ignoreDuplicates: true })

      if (dasError) {
        console.error("Erro específico na tabela das_payments:", dasError.message)
        setError(`Erro ao gerar guias DAS: ${dasError.message}`)
        return
      }
      ...
      console.log("Fluxo concluído com sucesso! Redirecionando...")
      ...
      console.error('Erro inesperado no processo de onboarding:', err)
    ...
    <div className="onboarding-page">
      <div className="onboarding-progress">
        {Array.from({ length: 4 }, (_, i) => (
          <div
            key={i}
            className={`onboarding-dot ${i + 1 <= step ? 'onboarding-dot--active' : ''}`}
          />
        ))}
      </div>
          ...
          >
            ...
            {step === 2 && (
              <Step2
                ...
                onCnpjChange={v => update('cnpj', v)}
                onBusinessNameChange={v => update('business_name', v)}
                error={error}
                onNext={next}
                onBack={() => setStep(1)}
              />
  ...
  return (
    <div className="onboarding-step">
      ...
      <p className="onboarding-step-desc">
        Opcional. Se informar, preenchemos seu nome empresarial automaticamente.
      </p>
      ...
      <Input
        ...
        suffix={
          <Button
            ...
            disabled={cleanCNPJ(cnpj).length !== 14}
          >
            Buscar
          </Button>
        }
      ...
      <Input
        ...
        placeholder="Maria Souza Design"
        value={businessName}
  ...
  return (
    <div className="onboarding-step">
      ...
      <div className="activity-options">
        {(Object.entries(MEI.ACTIVITY_TYPES) as [
          ActivityType,
          { label: string; das: number }
        ][]).map(([key, info]) => (
          <button
            key={key}
            type="button"
            className={`activity-option ${
              value === key ? 'activity-option--active' : ''
            }`}
            onClick={() => onChange(key)}
          >
            <div className="activity-option-label">{info.label}</div>
            <div className="activity-option-das">
              DAS mensal:{' '}
              <span>R$ {info.das.toFixed(2).replace('.', ',')}</span>
            </div>
          </button>
        ))}
      </div>
  ...
  return (
    <div className="onboarding-step">
      ...
      <div className="onboarding-summary">
        ...
        <SummaryRow
          label="Atividade"
          value={MEI.ACTIVITY_TYPES[data.activity_type].label}
        />
        <SummaryRow
          label="DAS mensal"
          value={`R$ ${MEI.ACTIVITY_TYPES[data.activity_type].das
            .toFixed(2)
            .replace('.', ',')}`}
          highlight
        />
      ...
      <p className="onboarding-das-note">
        Os 12 DAS de {new Date().getFullYear()} serão gerados automaticamente.
      </p>
      ...
      <div className="onboarding-actions">
        ...
        <Button onClick={onFinish} loading={saving}>
          Entrar no Faturei →
        </Button>
      </div>
    </div>
    ...
    <div className="summary-row">
      ...
      <span
        className={`summary-value ${
          highlight ? 'summary-value--highlight' : ''
        }`}
      >
        {value}
      </span>
